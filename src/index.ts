/**
 * ZResearch — a bundled research-methodology skill library for DeepSeek Harness.
 *
 * A self-contained Cordis plugin that registers ONE skill on ctx.skills. The
 * skill body (SKILL.md) plus its bundled references live under `skills/`, and
 * this module scans that directory, parses each SKILL.md, and serves it through
 * a skill provider so the stock dsh-tool-skill consumer turns it into the
...
// ---------------------------------------------------------------------------
// Plugin identity. `name` is the Cordis plugin id (matches cordis.patch.yml),
// and `inject` declares the host services this plugin reads.
// ---------------------------------------------------------------------------
export const name = 'za-ppt';
...
// ---------------------------------------------------------------------------
// Skill document validation (mirrors the shared SKILL.md field rules)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Library scan: `<root>/<name>/SKILL.md` bundles, one level deep.
// ---------------------------------------------------------------------------
...
class ZResearchSkillProvider implements SkillProvider {
...
export function apply(ctx: Context) {
  ctx.skills.registerProvider(
    () => new ZResearchSkillProvider('za-ppt', (message) => ctx.logger?.warn(message)),
  );
}
